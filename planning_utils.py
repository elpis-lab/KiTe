...
class SE2ControlPlanner:
    # bounds, control_bounds, model, belief, sampling, selection
    def __init__(
        self,
        bounds,
        control_bounds,
        ...
        self.set_up_planner(
            model, belief, active_sampling, active_selection, controls
        )
    ...
    def set_up_planner(
        self, model, belief, active_sampling, active_selection, controls
    ):
    ...
    def plan(
        ...
        # Set optimization objective if it is belief space,
        # which needs goal and goal region
...
class ControlBatchSampler(oc.ControlSampler):
    ...
    def __init__(
        self, space, c_space, model, control_list=None, cache_size=10000
    ):
...
def get_random_se2_states(
    n_data, pos_range=(-0.2, 0.2, -0.5, -0.9), euler_range=(-np.pi, np.pi)
):
...
def is_state_success(poses, goal, goal_region):
    ...
    return (in_x & in_y & in_yaw).tolist()
