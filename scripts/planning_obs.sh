...
for obj in "${objs[@]}"; do
    for n_data in "${n_datas[@]}"; do
        ...
        python scripts/planning_obs.py "$obj" "$model_type" 0 "$n_data" 0 1 0 "$predef_controls" "$n_reps" &
        echo

        # Plan for non-belief space
        echo "=== Planning $obj with $model_type $use_var $n_data 0 0 0 ==="
        python scripts/planning_obs.py "$obj" "$model_type" $use_var "$n_data" 0 0 0 "$predef_controls" "$n_reps" &
        echo
        ...
        for sampling in "${active_sampling[@]}"; do
            for selection in "${active_selection[@]}"; do
                ...
                python scripts/planning_obs.py "$obj" "$model_type" $use_var "$n_data" 1 $sampling $selection "$predef_controls" "$n_reps" &
                echo
            done
        done
        wait
