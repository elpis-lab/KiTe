...
from propagation.lie_se2 import (
    exp_se2,
    right_jacobian_se2,
    adjoint_se2,
    get_se2_vec,
...
class Propagation_SE2:
    ...
    def get_end_state(self):
        ...
        return get_se2_vec(self.mean), self.cov
...
if __name__ == "__main__":
    from propagation.lie_se2 import get_transform, inv_transform_se2, log_se2
    from propagation.plot_utils import plot_results_3d

    ########## Test Propagation_SE2 ##########
    # initial state
    t0 = get_transform((1.0, 1.0, np.deg2rad(60)))
    q0 = 1e-8 * np.eye(3)
    ...
    mean_inv = inv_transform_se2(mean)
    ...
    # Use Monte Carlo
    region_inv = inv_transform_se2(region)
    rel_ts = np.array(
